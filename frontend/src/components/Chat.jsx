...
export default function Chat() {
  ...
  // ✅ FAST Typing animation (like Gemini - instant appearance)
  const typeText = useCallback(
    (text, speed = 5) => // Much faster speed - 5ms per character
      new Promise((resolve) => {
        let i = 0;
  ...
  // ✅ Update title in backend + frontend
  ...
  // ✅ Toggle Supplier Mode - GREEN when active
  ...
  // ✅ Generate fallback supplier results
  const generateFallbackSupplierResults = (query) => {
    return [
      {
...
const sendMessage = async () => {
  ...
  try {
    ...
    if (supplierMode) {
      ...
      console.log("🛍️ Using supplier search for:", messageToSend);

      try {
        const res = await axios.post(
          `${API_BASE}/supplier/search`,
          { query: messageToSend, conversationId },
          { 
            headers: { 
        ...
        if (suppliers.length > 0) {
          assistantContent = `🔍 **Supplier Search Results for "${messageToSend}"**\n\n`;
          suppliers.forEach((supplier, index) => {
            assistantContent += `**${index + 1}. ${supplier.name}**\n`;
            assistantContent += `📍 ${supplier.country || 'N/A'} | ⭐ Match: ${supplier.matchScore || 'N/A'}%\n`;
          ...
          assistantContent = `❌ No suppliers found for "${messageToSend}". Try using words like manufacturer, supplier, or wholesaler.`;
        }

      } catch (apiError) {
        console.error("❌ Supplier API failed:", apiError);
        const fallbackSuppliers = generateFallbackSupplierResults(messageToSend);
        assistantContent = `🔍 **Supplier Search Results for "${messageToSend}"**\n\n*Note: Showing sample data (backend unavailable)*\n\n`;
        fallbackSuppliers.forEach((supplier, index) => {
          assistantContent += `**${index + 1}. ${supplier.name}**\n`;
          assistantContent += `📍 ${supplier.country} | ⭐ Match: ${supplier.matchScore}%\n`;
      ...
      try {
        ...
        console.error("❌ Chat API failed:", apiError);
        assistantContent = `I understand you're asking about "${messageToSend}". (Fallback mode active — backend unavailable.)`;
      }
    }

    // 💬 Type out assistant response with FAST animation
    await typeText(assistantContent, 3); // Even faster - 3ms per character
    ...
    // 🧱 Replace temporary message with final ones
    setMessages((prev) =>
      prev.filter((msg) => msg._id !== tempUserMsg._id)
          .concat([userMessage, assistantMessage])
    );

    // ✏️ Update conversation title if new
  ...
  // ✅ Function to detect and format URLs as clickable links
  ...
  const renderContent = (content) => {
    ...
    return parts.map((part, index) => {
      if (part.type === "code") {
        ...
        // ✅ Apply URL formatting to the final text
  ...
  return (
    <div className={`flex h-screen ${darkMode ? 'dark bg-gray-900 text-white' : 'bg-white text-gray-900'} relative overflow-hidden`}>
      ...
      >
        {sidebarOpen && (
          <>
            {/* Mobile Header */}
            {isMobile && (
              <div className={`p-4 flex items-center justify-between ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-gray-50 border-gray-200'}`}>
            ...
            {/* Header - Desktop */}
            {!isMobile && (
              <div className={`p-4 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-gray-50 border-gray-200'}`}>
      ...
      <div
        className={`flex-1 flex flex-col transition-all duration-300 ${sidebarOpen && !isMobile ? "ml-80" : "ml-0"
          } ${darkMode ? 'bg-gray-900' : 'bg-white'} scrollbar-hide`}
      >
        <div className={`px-4 md:px-6 py-4 flex items-center justify-between ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
          ...
          <div className="flex items-center gap-3">
            {/* Supplier Mode Toggle Button - GREEN when active */}
            <button
            ...
            <button
              ...
              className={`w-10 h-10 flex-shrink-0 rounded-full cursor-pointer transition-colors 
        ...
        {selectedConversation || messages.length > 0 ? (
          <>
            ...
            <div className={`flex-1 overflow-y-auto scrollbar-hide ${isMobile ? 'pb-20 mobile-messages-area' : ''}`}>
              {messages.length > 0 ? (
                <div className={`max-w-5xl mx-auto py-4 md:py-8 px-3 md:px-4 w-full ${isMobile ? 'mobile-message-container' : ''}`}>
                  {messages.map((msg) => (
                    <div
                      key={msg._id}
                        ...
                        className={`inline-block text-start px-1 py-1 md:px-2 md:py-2 rounded-2xl ${
                          msg.role === "user"
                            ? `${
                              ...
                              } ${isMobile ? "mobile-user-message max-w-[85%]" : "max-w-[80%]"}` 
                            : `${
                                darkMode
                                  ? "bg-transparent text-white rounded-bl-none"
                                  : "bg-transparent text-gray-800 rounded-bl-none"
                              } ${isMobile ? "mobile-assistant-message max-w-[90%]" : "max-w-[80%]"}`
                        }`}
                      >
                  ...
                  {/* Typing Animation - NO BACKGROUND COLOR */}
                  {isTyping && (
            ...
            {/* Input Area - NO ORANGE BORDER */}
            <div className={`${isMobile ? 'fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 p-4 z-50' : 'p-6 md:p-8'}`}>
              <div className="max-w-4xl mx-auto">
                <div className={`flex items-center border rounded-full px-4 py-3 md:px-6 md:py-4 ${
                  ...
                  <textarea
                    ...
                    placeholder={supplierMode ? "Search for suppliers, manufacturers, contractors..." : "Ask Gemini..."}
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center px-4 scrollbar-hide">
            <div className="max-w-2xl w-full">
              ...
              {/* Supplier Mode Toggle in Empty State - GREEN when active */}
              <div className="mb-6 md:mb-8 flex justify-center">
                ...
                >
                  <i className={`ri-${supplierMode ? 'store-2-fill' : 'store-2-line'} text-lg`}></i>
                  <span>
                    {supplierMode ? 'Supplier Mode ON' : 'Enable Supplier Mode'}
                  </span>
                </button>
              </div>

              {/* Centered Input Area - NO ORANGE BORDER */}
      ...
      {/* Add CSS for mobile optimizations and link styling */}
      <style>{`
        .scrollbar-hide {
